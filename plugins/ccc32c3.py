...
class ccc32c3(Plugin):
    ...
    def get_plays(self, hall):
        now_playing = json_data['schedule']['conference']['days'][0]['rooms'][
            hall][0]
        now_playing['date'] = "2099-12-31T23:59:59+01:00"

        next_playing = json_data['schedule']['conference']['days'][0]['rooms'][
            hall][0]
        ...
        for x in json_data['schedule']['conference']['days']:
            ...
            for event in x['rooms'][hall]:
                ...
                if(time_diff.days >= 0):
                    if(datetime.datetime.strptime(event['date'][0:-6],
                                                  "%Y-%m-%dT%H:%M:%S") <
                       datetime.datetime.strptime(next_playing['date'][0:-6],
                                                  "%Y-%m-%dT%H:%M:%S")):
                        next_playing = event
    ...
    def onPrivmsg(self, irc, msg, channel, user):
        ...
        if msg.find("now") != -1:
            if last_ccc32c3 < (time() - 60):
                ...
                irc.msg(channel, ("Hall 1: " + self.conv(now['date']) + " " +
                                  now['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall 2")
                irc.msg(channel, ("Hall 2: " + self.conv(now['date']) + " " +
                                  now['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall G")
                irc.msg(channel, ("Hall G: " + self.conv(now['date']) + " " +
                                  now['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall 6")
                irc.msg(channel, ("Hall 6: " + self.conv(now['date']) + " " +
                                  now['title']).encode("utf-8", "ignore"))
        ...
        if msg.find("next") != -1:
            if last_ccc32c3 < (time() - 60):
                now, next = self.get_plays("Hall 1")
                irc.msg(channel, ("Hall 1: " + self.conv(next['date']) + " " +
                                  next['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall 2")
                irc.msg(channel, ("Hall 2: " + self.conv(next['date']) + " " +
                                  next['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall G")
                irc.msg(channel, ("Hall G: " + self.conv(next['date']) + " " +
                                  next['title']).encode("utf-8", "ignore"))

                now, next = self.get_plays("Hall 6")
                irc.msg(channel, ("Hall 6: " + self.conv(next['date']) + " " +
                                  next['title']).encode("utf-8", "ignore"))
